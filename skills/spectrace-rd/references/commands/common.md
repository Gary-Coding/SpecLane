...
- `/st:bridge` 之后只能提示人工审核 `todo.md`，审核后 `/st:apply`。
- `/st:apply` 之前必须已经完成 bridge 且用户已审核 todo。
- 通知只能由 `run-workflow.py verify` 发送，AI 禁止直接调用飞书或 PushPlus webhook。

## 最小上下文

- `/st:propose`：`workspace.yml`、`demand_file`、必要 `reference_files` 摘要。
- `/st:bridge`：`tasks.md`、bridge context、`todo_file`。
- `/st:plan`：`todo.md`、`discovery-summary` 或脚本生成的 plan 产物。
- `/st:apply`：优先 `todo.md`、`plan-summary.json`、目标代码文件。
- `/st:review` / `/st:verify`：优先读取 summary 和脚本报告，不展开长 diff 或长日志。
