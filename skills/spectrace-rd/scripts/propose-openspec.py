...
def main() -> None:
    ...
    if not explicit_change_name:
        raise SystemExit("缺少 OpenSpec change 名称。请使用 /st:propose <change-name> 显式指定。")
    ...
    payload = {
        ...
        "allowed_next_after_completion": ["/st:bridge"],
        "forbidden_next_after_completion": ["/st:plan", "/st:apply"],
        "final_reply_constraint": "代码未修改。下一步只能执行 /st:bridge。",
    }
    ...
    print("allowed_next_after_completion=/st:bridge")
    print("forbidden_next_after_completion=/st:plan,/st:apply")
    print("final_reply_must=代码未修改。下一步只能执行 /st:bridge。")
