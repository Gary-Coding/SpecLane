...
def load_skill_config() -> dict[str, Any]:
    ...
    if config.get("version") != 1:
        raise ValueError("~/.spectrace/skill-config.yml 中的 version 必须为 1。")

    notification = config.get("notification", {})
    if notification in ("", None):
        notification = {}
    if not isinstance(notification, dict):
        raise ValueError("~/.spectrace/skill-config.yml 中的 notification 必须是对象。")

    pushplus = notification.get("pushplus", {})
    if pushplus in ("", None):
        pushplus = {}
    if not isinstance(pushplus, dict):
        raise ValueError("~/.spectrace/skill-config.yml 中的 notification.pushplus 必须是对象。")
    ...
    if not isinstance(ordinary_raw, dict):
        raise ValueError("~/.spectrace/skill-config.yml 中的 notification.pushplus.ordinary 必须是对象。")
    if not isinstance(feishu_raw, dict):
        raise ValueError("~/.spectrace/skill-config.yml 中的 notification.feishu 必须是对象。")
    ...
    if not ordinary_channel:
        raise ValueError("~/.spectrace/skill-config.yml 中的 notification.pushplus.ordinary.channel 不能为空。")
    if ordinary_enabled and not pushplus_token:
        raise ValueError("启用 PushPlus 通知时，~/.spectrace/skill-config.yml 中的 notification.pushplus.token 不能为空。")
    if feishu_enabled and not feishu_webhook_url:
        raise ValueError("启用飞书通知时，~/.spectrace/skill-config.yml 中的 notification.feishu.webhook_url 不能为空。")
...
def load_workspace_config(workspace: Path | None = None) -> dict[str, Any]:
    ...
    if config["workflow_source"] == "openspec":
        ...
        writeback_dir = resolve_workspace_path(root, openspec_raw.get("writeback_dir", change_dir / "spectrace"))
        openspec = {
...
def artifacts_dir(config: dict[str, Any]) -> Path:
    return Path(str(config["__workspace_root"])).resolve() / ".spectrace"
...
def ensure_plan_can_run(config: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if phase in ("plan", "wait_confirm_plan"):
        return active
    raise RuntimeError(
        f"当前已有活跃 session 正在交付中，禁止重新执行 /st:plan："
        f"session_id={session.get('session_id', '')}, phase={phase}。"
        "请继续当前 /st:apply 链路，不要创建新的 plan/session。"
    )
...
ST_PHASE_ALLOWED_NEXT: dict[str, list[str]] = {
    ...
    "planned": ["/st:apply"],
    "implementing": [],
    "self_checked": ["/st:review"],
    "reviewed": ["/st:verify", "/st:apply"],
    "verified": ["/st:archive-check"],
    "archive_ready": ["/st:archive"],
    "archived": [],
    "blocked": ["/st:apply", "/st:verify"],
}

TODO_PHASE_ALLOWED_NEXT: dict[str, list[str]] = {
    "draft": ["/st:init", "/st:plan", "/st:apply"],
    "planned": ["/st:apply"],
    "implementing": [],
    "self_checked": ["/st:review"],
    "reviewed": ["/st:verify", "/st:apply"],
...
RUN_COMMAND_TO_ST_COMMAND: dict[str, str] = {
    ...
    "plan": "/st:plan",
    "apply": "/st:apply",
    "start-implement": "/st:apply",
    "finish-implement": "/st:apply",
    "review": "/st:review",
    "verify": "/st:verify",
    "prepare-archive-openspec": "/st:archive-check",
    "archive-openspec": "/st:archive",
}
...
ST_COMMAND_TO_RUN_COMMAND: dict[str, str] = {
    ...
    "/st:review": "review",
    "/st:verify": "verify",
    "/st:archive-check": "prepare-archive-openspec",
    "/st:archive": "archive-openspec",
    "/st:status": "status",
}
...
def validate_standard_session(config: dict[str, Any], require_notification: bool = False) -> dict[str, Any]:
    errors: list[str] = []
    if current_session_is_stale(config):
        errors.append("current-session.json 指向旧 output_dir，请重新执行 /st:plan 创建当前需求的标准会话。")
        return {"valid": False, "errors": errors}
    session_meta = _safe_current_session_meta(config)
    if not session_meta:
        errors.append("缺少当前 session，请先执行 /st:plan。")
        return {"valid": False, "errors": errors}
    ...
    if workflow_source(config) != "openspec":
        ...
        if run_command in ("propose-openspec", "bootstrap-openspec", "prepare-archive-openspec", "archive-openspec"):
            ...
            if not todo_path(config).exists():
                errors.append("缺少 todo_file，请先执行 /st:init 或补充 todo.md。")
            ...
            if phase not in ("planned", "implementing", "blocked"):
                errors.append("当前状态不允许进入实现，请先执行 /st:plan。")
        elif run_command == "finish-implement":
            if phase != "implementing":
                errors.append("当前状态不允许完成实现，必须先通过 /st:apply 进入 implementing。")
...
def validate_openspec_change_name(change_name: str) -> str:
    ...
    if not normalized:
        raise ValueError("OpenSpec change 名称不能为空。请使用 /st:propose <change-name> 显式指定。")
    ...
    openspec.update(
        {
            ...
            "writeback_dir": str(change_dir / "spectrace"),
        }
    )
    config["openspec"] = openspec
    return config
...
def lark_cli_install_message() -> str:
    return "\n".join(
        [
            ...
            "安装和授权完成后重新执行 /st:propose <change-name>。",
        ]
    )
...
def collect_openspec_cli_context(config: dict[str, Any], include_apply: bool = False, include_archive: bool = False) -> dict[str, Any]:
    ...
    if not change_name:
        context["error"] = "missing active OpenSpec change; run /st:propose <change-name> first"
        return context
...
def ensure_workflow_inputs(config: dict[str, Any], *, allow_bridge_write: bool = False) -> dict[str, Any]:
    ...
    if not change_name:
        raise ValueError(
            "缺少当前 OpenSpec change。请先执行 /st:propose <change-name> 记录 active change；"
            "不需要把 workspace.yml 改成绝对路径，也不需要显式配置 openspec.change_dir。"
        )
    tasks_file = openspec_tasks_path(config)
    if not tasks_file.exists():
        raise FileNotFoundError(
            f"OpenSpec tasks 文件不存在：{tasks_file}。请确认已执行 /st:propose {change_name} 并生成 tasks.md；"
            "不要手工生成桥接产物。"
        )
    ...
    if not allow_bridge_write:
        if not existing.strip():
            raise FileNotFoundError(
                f"桥接 todo 文件不存在：{todo_file}。OpenSpec 模式下只有显式执行 /st:bridge "
                "才允许从 tasks.md 生成 todo.md；/st:init、/st:propose、/st:plan、/st:apply 都不能自动生成桥接 todo。"
            )
