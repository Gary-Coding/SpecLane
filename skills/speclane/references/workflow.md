...
其中 `demand_file` 可选，主要作为 `/sl:propose` 的原始需求输入。以上路径可以使用相对路径或绝对路径。相对路径按当前工作空间根目录解析。
...
`~/.speclane/skill-config.yml` 可以包含可选通知：
...
如果 `~/.speclane/skill-config.yml` 不存在：
...
OpenSpec change 名称必须通过 `/sl:propose <change-name>` 显式指定。工作流不会从 `vars.demand_name`、需求文件名或需求标题推导 change 名称。`openspec.change_dir` / `openspec.change_name` 只作为兼容旧配置的字段，不建议新增配置。
...
- `openspec` 模式下，`/sl:propose` 优先读取它生成或完善 change
...
- `openspec` 模式下，`/sl:propose` 必须读取真实存在的 `reference_files`，并把内容写入 `propose-input.json` / `propose-input.md`，作为生成 `proposal.md`、`design.md`、`tasks.md` 的上下文
...
- `<output_dir>/<session_id>/unit-test.md`
...
- `<workspace>/.speclane/sessions/<session_id>/notification.json`

通知验收规则：

- `notification.json` 是唯一通知证据
- `status.json.notification_status` 只是摘要，不能单独作为通知成功依据
- 通知只能由 `run-workflow.py verify` 调用 `rd-verify.py` 和 `common.notify_workflow_result()` 发送
...
- OpenSpec 模式下，`sl-state.json` 是脚本状态机，记录 `phase`、`allowed_next`、`current_change`、`last_command` 和关键产物路径
- `/sl:propose` 后状态为 `proposed`，只允许 `/sl:bridge`
- `/sl:bridge` 后状态为 `bridged`，允许 `/sl:apply` 或 `/sl:plan`
- `/sl:plan` 后状态为 `planned`，只允许 `/sl:apply`
- `/sl:verify` 通过后状态为 `verified`，只允许 `/sl:archive-check`
- `/sl:archive-check` 通过后状态为 `archive_ready`，只允许 `/sl:archive`
...
- `/sl:apply` 的实现阶段要求 AI 同步补充或更新必要单元测试
- `plan` 会自动执行 `discover`，`finish-implement` 会自动执行 `unit-test` 和 `self-check`
- `workflow_source=openspec` 时，只有用户显式执行 `/sl:bridge` 才会桥接 OpenSpec `tasks.md` 到 `todo_file`
- `workflow_source=openspec` 时，`init` / `plan` 只校验已有桥接 todo，不能自动创建或重写 `todo_file`
- `workflow_source=openspec` 时，`openspec-propose` 优先调用 OpenSpec CLI 创建 change、读取 status 和 artifact instructions
- `workflow_source=openspec` 时，`review` / `verify` 会自动把执行摘要写回 `openspec.writeback_dir`
- OpenSpec 长期规格归档建议显式执行 `openspec-archive-check` 与 `openspec-archive`；归档检查会结合 OpenSpec CLI status 与 speclane 的 spec baseline 冲突检测
- `openspec-archive-check` 会检测 spec baseline 是否发生变化；只有 `merge_mode=safe_merge` 才允许后续归档
- `auto` 模式下，除非进入硬阻塞，否则不能在对话里要求用户批准继续
- 工作流总耗时按当前会话开始到 verify 收口结束的真实墙钟时间计算
- AI 禁止编辑 `<workspace>/workspace.yml`
- AI 禁止直接写 `.speclane/current-session.json`、`.speclane/sessions/**/status.json`、`plan.json`、`unit-test.json`、`review.json`、`verify.json`、`notification.json` 和 output 下的标准 Markdown 报告；这些标准产物只能由脚本生成
...
- 如果 session 已经被标记为 `done`，但缺少 `verify.json`、`notification.json` 或输出目录下的 Markdown 报告，说明前一次没有走标准收口，应通过 `/sl:verify` 重新执行标准验证收口
