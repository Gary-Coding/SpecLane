...
def main() -> None:
    ...
    if workflow_source(config) != "openspec":
        raise SystemExit("当前 workspace.yml 未启用 OpenSpec 模式，无需执行 openspec-archive。")
    ...
    write_managed_json(config, archive_target / "speclane" / "archive-result.json", payload)
    write_managed_text(config, archive_target / "speclane" / "archive-result.md", build_markdown(payload))
    update_sl_state(
        config,
        phase="archived",
        lasl_command="/sl:archive",
        artifacts={
            "archive_result": str(archive_target / "speclane" / "archive-result.json"),
            "archived_to": str(archive_target),
        },
    )
