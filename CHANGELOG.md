...
- 拆分 `/st:*` 协议到 `references/commands/*`，删除旧单体协议入口，AI 按命令读取最小协议上下文，降低固定 token 消耗。
...
- `route-st` 增加 `--json` 摘要输出，并增加跨进程 workflow lock，降低并发或重复 `/st:apply` 的 session 风险。
- OpenSpec 回写增加 `openspec_hashes` 和独立 `task-mapping.json`，archive-check 会检测 proposal/design/tasks/specs 的 hash 漂移。
- 增强 `spectrace doctor --fix`，可同步 skill 并补齐多平台命令模板。
...
- 修复计划生成失败或中断后重复 `/st:apply` 会创建多个空 session/output 目录的问题。

## 0.1.5

- 修复重复执行 `/st:plan` 会创建多个 session 的问题：计划阶段复用当前 session，交付中重复 plan 会被拒绝。
- 增加 E2E 测试，覆盖模板 CLI、OpenSpec 状态机与桥接、todo auto 会话、verify 长日志截断。
- 增加 `spectrace templates`、`spectrace template show`、`spectrace template copy`，支持复制内置 `workspace.yml` 模板。
...
- 压缩 `SKILL.md` 为轻量入口，详细规则改为按需读取 references，降低每次 `/st:*` 固定上下文消耗。
- 快捷命令模板改为极简形式，减少 Claude / Codex slash command 触发时的重复提示词。
- `/st:propose` 不再把 `reference_files` 全文复制进 `propose-input.json` / `propose-input.md`，改为路径、sha256、标题和摘要片段。
...
- `spectrace init` 在 `openspec` 模式下默认尝试执行 `openspec init . --tools codex,claude`。
- `spectrace init` 自动生成工作区 `.claude/commands/st/*` 快捷命令。
- 当初始化时选择安装 Codex skill，同步生成 `~/.codex/prompts/st-*.md` 快捷提示。
...
- 增加 `se` / `spectrace` npm CLI 入口。
- 增加交互式初始化向导。
- 增加 `spectrace doctor`、`se install`、`spectrace sync`、`se version`。
