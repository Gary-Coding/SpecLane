...
这份文档用于从零准备一个可运行的 `spectrace + OpenSpec` 工作区。目标是让用户只需要准备目录、配置文件和提示词，后续由 AI 根据 skill 调用受控脚本推进流程。
...
- `demands/<demand_name>/` 放当前需求输入、桥接 todo 和输出报告。
...
```text
~/.codex/skills/spectrace-rd
```

Claude skill 目录：

```text
~/.claude/skills/spectrace-rd
```

如果是从本项目同步：

```bash
rsync -a --delete spectrace-rd/ ~/.codex/skills/spectrace-rd/
rsync -a --delete spectrace-rd/ ~/.claude/skills/spectrace-rd/
```

同步后可以检查：

```bash
python3 -m py_compile ~/.codex/skills/spectrace-rd/scripts/*.py
python3 -m py_compile ~/.claude/skills/spectrace-rd/scripts/*.py
```

也可以使用项目自带引导脚本安装并初始化工作区。默认是一步一步的交互式向导：

```bash
cd /path/to/spectrace
python3 scripts/spectrace-setup.py
```
...
```text
ai-workspace/.claude/commands/st/
```
...
```bash
npx @gary-coding/spectrace init
```
...
```bash
spectrace init      # 交互式安装 skill 并初始化工作区
spectrace doctor    # 检查本机环境和 workspace.yml
spectrace templates # 查看内置 workspace.yml 模板
spectrace install   # 安装 skill 到 Codex / Claude
spectrace sync      # 强制同步最新 skill 到 Codex / Claude
spectrace migrate   # 补齐旧工作区缺失配置
spectrace version   # 查看版本
```
...
```bash
spectrace templates
spectrace template copy openspec-auto \
  --workspace /path/to/ai-workspace \
  --demand-name 10-your-demand \
  --code-path ../code
```
...
- `package.json` 声明 `bin.spectrace` 和 `bin.se`。
- bin 脚本转发到 `scripts/spectrace-setup.py`。
...
如果没有快捷命令，也可以直接把 `/st:*` 提示词发给 AI。skill 仍然能识别并执行。
...
```text
ai-workspace/demands/10-your-demand/
└── 需求.md
```
...
```text
/st:propose your-change-name
请根据当前 workspace 的 demand_file 生成或完善 OpenSpec change。
不要改代码。
回复语言默认跟随用户输入；如需指定语言，可补充“请使用中文”或 “Please respond in English”。
```
...
```text
demands/10-your-demand/todo.md
```
...
### `/st:propose` 会不会自动生成 todo？

不会。`/st:propose` 只能生成 OpenSpec change 文档。只有显式执行 `/st:bridge` 才允许生成或重写 `todo_file`。
...
不要手工把 `tasks.md` 同步到 `todo.md`。正确做法是补充需求，重新执行 `/st:propose <change-name>`，然后重新 `/st:bridge`。

### 可以跳过 `/st:bridge` 直接 `/st:apply` 吗？

OpenSpec 模式下不可以。`/st:apply` 前必须先有经过人工审核的桥接 todo。
