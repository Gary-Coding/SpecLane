...
`speclane` 内置了一组 `workspace.yml` 模板，用于减少团队初始化成本，并让不同项目按统一结构接入工作流。

## 查看模板

```bash
speclane templates
speclane template show openspec-auto
```
...
```bash
speclane template copy openspec-auto --workspace /path/to/ai-workspace --force
```
