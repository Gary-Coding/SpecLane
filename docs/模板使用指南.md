...
`spectrace` 内置了一组 `workspace.yml` 模板，用于减少团队初始化成本，并让不同项目按统一结构接入工作流。

## 查看模板

```bash
spectrace templates
spectrace template show openspec-auto
```
...
```bash
spectrace template copy openspec-auto --workspace /path/to/ai-workspace --force
```
