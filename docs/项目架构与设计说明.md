...
### 4.1 `skills/speclane-rd/SKILL.md`

`skills/speclane-rd/SKILL.md` 是唯一 skill 入口，定义 AI 行为协议、命令导航和工作流约束。
...
AI 使用这个 skill 时，首先读取 `SKILL.md`，再按需读取同目录 `references/`，执行动作时统一调用 `scripts/`。
...
### 4.4 `skills/speclane-rd/assets/`

`skills/speclane-rd/assets/` 存放配置示例和结构化 schema：
...
### 4.5 `skills/speclane-rd/scripts/`

`skills/speclane-rd/scripts/` 是 RD 阶段的本地 runtime。统一入口是：

```text
skills/speclane-rd/scripts/run-workflow.py
```

### 4.6 多需求实例

同一工作区可以通过 demand 实例隔离多个需求：
...
```yaml
version: 1
demand_name: demand-a
workflow_source: openspec
mode: auto
demand_file: demands/demand-a/需求.md
todo_file: demands/demand-a/todo.md
output_dir: demands/demand-a/output
reference_files: []
openspec:
  change_name: add-user-phone-filter
```

命令可以通过 `--demand <demand-name>` 显式选择需求；没有显式指定时，脚本优先读取 `.speclane/active-demand.yml`。没有 active demand 时，保持旧单需求工作区行为。
...
```text
demands/7-add-phone-filter/需求.md
```
...
4. [skills/speclane-rd/references/workflow.md](../skills/speclane-rd/references/workflow.md)

不建议新手一开始直接研究 `skills/speclane-rd/scripts/`。先跑通一次完整需求，再看脚本实现会更清楚。
