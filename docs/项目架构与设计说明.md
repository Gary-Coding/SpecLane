...
本文面向首次接触 `spectrace` 的开发者，说明项目当前阶段、核心思想、架构分层和后续演进方向。

## 1. 项目当前阶段

`spectrace` 当前处于“可用工作流内核 + Skill 分发”的阶段。
...
它暂时不是完整的平台产品，也不是一个独立的 Web 应用。当前的定位更接近“AI 工程交付操作系统的本地内核”：用户通过 AI 输入 `/st:*` 阶段命令，AI 按 skill 规则调用本项目脚本，脚本负责稳定地产出状态、报告和门禁证据。

当前边界也需要明确：

- 代码实现动作仍由 AI 根据 `plan.json` 和代码上下文完成，不是由 Python 脚本自动改业务代码。
- OpenSpec 已接入到 change 生命周期，但还不是完全替代 OpenSpec 官方 CLI 的产品。
- `/st:*` 是发给 AI 的协议，不是 shell 命令，也不是 OpenSpec 官方命令。
- 脚本主要负责流程编排、状态管理、报告生成、验证执行和 OpenSpec 桥接。
- 当前已提供 `/st:*` 的脚本路由入口，AI 收到命令后应优先通过统一入口解析并推进，不应自行组合状态文件。
...
`spectrace` 的核心目标是把一次需求迭代拆成稳定阶段，并把关键事实落到文件系统：
...
`SKILL.md` 和 `references/` 定义 AI 应该如何理解用户意图，尤其是 `/st:*` 命令。
...
### 4.1 `skills/spectrace-rd/SKILL.md`
...
- `commands/`：按命令拆分的 `/st:*` 专属协议，AI 只读取当前命令需要的分片
...
- `/st:propose <change-name>` 阶段优先调用 OpenSpec CLI 创建指定 change。
...
项目推荐用户通过 `/st:*` 与 AI 交互，由 AI 根据 skill 协议调用脚本。这样可以保留自然语言协作体验，同时让底层执行稳定可追踪。
...
2. 用户发 `/st:propose <change-name>`。
3. AI 通过 `run-workflow.py route-st --command-text "/st:propose <change-name>"` 准备 OpenSpec 输入，并生成或完善 `proposal.md`、`design.md`、`tasks.md`。
4. 用户发 `/st:bridge`。
5. AI 通过 `run-workflow.py route-st --command-text "/st:bridge"` 把 `tasks.md` 转成 `todo.md`。
6. 用户审核 `todo.md` 后发 `/st:apply`。
...
这里的“单一 OpenSpec”指团队主要围绕 OpenSpec change、spec、tasks、archive 来管理需求变更，但不额外引入 `spectrace` 的计划、审查、验证和会话产物。

### 11.1 优点

相比只使用 OpenSpec，`spectrace + OpenSpec` 混合工作流的优势主要在交付阶段。

第一，交付过程更可控。OpenSpec 更擅长描述“规格如何变化”，但不会天然约束 AI 在代码实现阶段如何做计划、自查、审查和验证。`spectrace` 在 `tasks.md` 后增加 `todo.md` 桥接、`plan.json`、`review.json`、`verify.json`，让交付阶段有明确门禁。

第二，更适合复杂存量系统。供应链中台、多服务、多仓库项目里，需求往往不只是改一个 spec，还要定位服务、确认调用链、识别测试入口、判断计划外改动。`spectrace` 的 `discover -> plan -> review -> verify` 对这类场景更直接。

第三，更适合 AI 编码协作。OpenSpec 主要管理需求和规格，`spectrace` 进一步约束 AI 怎么读配置、怎么找代码、怎么记录状态、什么时候停止、什么时候自动推进。
...
```text
只管规格治理：OpenSpec
规格治理 + AI 交付闭环：spectrace + OpenSpec
只想快速让 AI 按 todo 写代码：spectrace todo 模式
```

## 12. 新手快速入门

新手不要一开始理解所有脚本。先按“一个工作空间、一份配置、一组 `/st:*` 提示词”来用。
...
3. [docs/st命令协议.md](./st命令协议.md)
4. [skills/spectrace-rd/references/workflow.md](../skills/spectrace-rd/references/workflow.md)
...
`spectrace` 的核心不是“再写一个 AI 编码提示词”，而是把 AI 编码变成一个可配置、可追踪、可审查、可验证、可归档的工程工作流。
