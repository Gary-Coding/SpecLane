...
```bash
spectrace doctor --workspace <workspace>
spectrace doctor --workspace <workspace> --fix
```

`--fix` 会同步本机 skill，并补齐 Claude、Codex、Cursor、Trae、Kimi 的 `/st:*` 快捷命令模板。
...
- 路径包含空格时，建议先用 `spectrace doctor` 检查解析结果。
