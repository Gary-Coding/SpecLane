...
- 目标代码仓库已经在本地
- 本地可用 `python3`
- 已存在 `~/.speclane/skill-config.yml`

如果 `~/.speclane/skill-config.yml` 不存在，第一次初始化时会自动生成模板，并暂停流程，等你补全。
...
OpenSpec change 名称不会从 `demand_name` 推导，必须在 `/sl:propose <change-name>` 中显式指定。
...
如果审核时发现桥接 todo 和实际业务有偏差，不要进入 `/sl:apply`。先判断偏差来自哪里。
...
如果原始需求本身不完整，先补充 `demand_file` 或在提示词中说明补充信息，再重新执行 `/sl:propose` 修正当前 change。
...
当前桥接 todo 已审核通过。
如果没有硬阻塞，自动推进到实现、自查、审查和验证。
verify 通过后继续检查归档条件；如果结果为 safe_merge，状态进入 archive_ready，下一步再执行 /sl:archive。
```
...
```text
/sl:propose add-user-phone-filter-follow-up
已归档需求发现后续遗漏：
手机号筛选需要同时兼容脱敏手机号字段，否则部分历史用户无法被查到。
...
- 已经 `verify done`，但还没归档：补当前 OpenSpec change，再重新 `/sl:bridge`，人工审核 todo 后执行 `/sl:apply`
...
这一节用于快速查阅命令顺序。`/sl:*` 是发给 AI 的工作流指令，不是 shell 命令。
...
-> /sl:apply
-> /sl:archive-check
-> /sl:archive 满足 safe_merge 时
```

| 顺序 | 命令 | 作用 | 完成后允许的下一步 |
| --- | --- | --- | --- |
| 1 | `/sl:init` | 初始化工作空间，检查配置、目录和输入 | `/sl:propose <change-name>` |
| 2 | `/sl:propose <change-name>` | 生成或完善指定 OpenSpec change，只处理规格，不改代码 | `/sl:bridge` |
| 3 | `/sl:bridge` | 把 OpenSpec `tasks.md` 桥接成 `todo.md` | 人工审核 todo.md 后 `/sl:apply` |
| 4 | `/sl:plan` | 生成交付计划，不改代码 | `/sl:apply` |
| 5 | `/sl:apply` | 进入实现、自查、审查、验证 | `/sl:archive-check` |
| 6 | `/sl:review` | 单独执行或重跑代码审查 | `/sl:verify` 或 `/sl:apply` 修复 |
| 7 | `/sl:verify` | 单独执行或重跑验证 | 通过后 `/sl:archive-check` |
| 8 | `/sl:archive-check` | 检查是否满足 OpenSpec 归档条件 | 满足 safe_merge 后 `/sl:archive` |
| 9 | `/sl:archive` | 执行 OpenSpec 归档 | 结束 |
| - | `/sl:status` | 查看当前工作流状态 | 按状态提示下一步 |

关键门禁：

- `/sl:propose` 后不能直接 `/sl:apply`
- `/sl:bridge` 后必须先人工审核 todo.md
- 人工审核 todo.md 后直接用 `/sl:apply` 进入交付
- `/sl:apply` 之前必须已经完成 `/sl:bridge` 并审核 todo.md
- `/sl:archive` 之前必须先 `/sl:archive-check`
- 飞书通知必须通过 `/sl:verify` 的标准模板发送，不能让 AI 手工拼 webhook

OpenSpec 模式下还有脚本状态机：

```text
.speclane/sl-state.json
```

脚本会记录 `phase` 和 `allowed_next`。例如 `/sl:propose` 后 `phase=proposed`，只允许 `/sl:bridge`；如果此时直接执行 `/sl:apply`，脚本会拒绝。
...
| 顺序 | 命令 | 作用 |
| --- | --- | --- |
| 1 | `/sl:init` | 初始化工作空间，检查 `workspace.yml` 和 `todo.md` |
| 2 | `/sl:plan` | 基于 `todo.md` 生成计划，不改代码 |
| 3 | `/sl:apply` | 按计划实现、自查、审查和验证 |
| 4 | `/sl:review` | 单独执行或重跑审查 |
| 5 | `/sl:verify` | 单独执行或重跑验证 |
| - | `/sl:status` | 查看当前状态 |

`todo + auto` 可以从 `/sl:apply` 开始，但前提是 `workspace.yml` 和 `todo.md` 都有效。

### 10.3 最常用的几条命令

如果你不想记全部命令，先记这几条就够了：

- `/sl:propose <change-name>`：生成或完善指定 OpenSpec change
- `/sl:bridge`：把 `tasks.md` 转成桥接 todo
- `/sl:plan`：只做计划
- `/sl:apply`：进入交付
- `/sl:archive-check`：检查是否满足归档条件
- `/sl:archive`：执行归档

如果 AI 收到 `/sl:*` 后没有进入工作流，而是回复“在吗”“有什么可以帮你”这类普通聊天内容，说明当前模型没有加载 `speclane-rd` skill，或者没有识别 `/sl:*` 协议。处理方式：

```text
请使用 speclane-rd skill 处理下面的 /sl:* 工作流命令。
...
如果仍然没有触发，需要确认当前 AI 客户端已经安装并启用了 `speclane-rd` skill。
