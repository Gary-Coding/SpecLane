...
- 本地已经安装好 `spectrace-rd`
- 目标代码仓库已经在本地
- 本地可用 `python3`
- 已存在 `~/.spectrace/skill-config.yml`

如果 `~/.spectrace/skill-config.yml` 不存在，第一次初始化时会自动生成模板，并暂停流程，等你补全。
...
OpenSpec change 名称不会从 `demand_name` 推导，必须在 `/st:propose <change-name>` 中显式指定。
...
如果审核时发现桥接 todo 和实际业务有偏差，不要进入 `/st:apply`。先判断偏差来自哪里。
...
如果原始需求本身不完整，先补充 `demand_file` 或在提示词中说明补充信息，再重新执行 `/st:propose` 修正当前 change。
...
当前桥接 todo 已审核通过。
如果没有硬阻塞，自动推进到实现、自查、审查和验证。
verify 通过后继续检查归档条件；如果结果为 safe_merge，状态进入 archive_ready，下一步再执行 /st:archive。
```
...
```text
/st:propose add-user-phone-filter-follow-up
已归档需求发现后续遗漏：
手机号筛选需要同时兼容脱敏手机号字段，否则部分历史用户无法被查到。
...
- 已经 `verify done`，但还没归档：补当前 OpenSpec change，再重新 `/st:bridge`，人工审核 todo 后执行 `/st:apply`
...
这一节用于快速查阅命令顺序。`/st:*` 是发给 AI 的工作流指令，不是 shell 命令。
...
-> /st:apply
-> /st:archive-check
-> /st:archive 满足 safe_merge 时
```

| 顺序 | 命令 | 作用 | 完成后允许的下一步 |
| --- | --- | --- | --- |
| 1 | `/st:init` | 初始化工作空间，检查配置、目录和输入 | `/st:propose <change-name>` |
| 2 | `/st:propose <change-name>` | 生成或完善指定 OpenSpec change，只处理规格，不改代码 | `/st:bridge` |
| 3 | `/st:bridge` | 把 OpenSpec `tasks.md` 桥接成 `todo.md` | 人工审核 todo.md 后 `/st:apply` |
| 4 | `/st:plan` | 生成交付计划，不改代码 | `/st:apply` |
| 5 | `/st:apply` | 进入实现、自查、审查、验证 | `/st:archive-check` |
| 6 | `/st:review` | 单独执行或重跑代码审查 | `/st:verify` 或 `/st:apply` 修复 |
| 7 | `/st:verify` | 单独执行或重跑验证 | 通过后 `/st:archive-check` |
| 8 | `/st:archive-check` | 检查是否满足 OpenSpec 归档条件 | 满足 safe_merge 后 `/st:archive` |
| 9 | `/st:archive` | 执行 OpenSpec 归档 | 结束 |
| - | `/st:status` | 查看当前工作流状态 | 按状态提示下一步 |

关键门禁：

- `/st:propose` 后不能直接 `/st:apply`
- `/st:bridge` 后必须先人工审核 todo.md
- 人工审核 todo.md 后直接用 `/st:apply` 进入交付
- `/st:apply` 之前必须已经完成 `/st:bridge` 并审核 todo.md
- `/st:archive` 之前必须先 `/st:archive-check`
- 飞书通知必须通过 `/st:verify` 的标准模板发送，不能让 AI 手工拼 webhook

OpenSpec 模式下还有脚本状态机：

```text
.spectrace/se-state.json
```

脚本会记录 `phase` 和 `allowed_next`。例如 `/st:propose` 后 `phase=proposed`，只允许 `/st:bridge`；如果此时直接执行 `/st:apply`，脚本会拒绝。
...
| 顺序 | 命令 | 作用 |
| --- | --- | --- |
| 1 | `/st:init` | 初始化工作空间，检查 `workspace.yml` 和 `todo.md` |
| 2 | `/st:plan` | 基于 `todo.md` 生成计划，不改代码 |
| 3 | `/st:apply` | 按计划实现、自查、审查和验证 |
| 4 | `/st:review` | 单独执行或重跑审查 |
| 5 | `/st:verify` | 单独执行或重跑验证 |
| - | `/st:status` | 查看当前状态 |

`todo + auto` 可以从 `/st:apply` 开始，但前提是 `workspace.yml` 和 `todo.md` 都有效。

### 10.3 最常用的几条命令

如果你不想记全部命令，先记这几条就够了：

- `/st:propose <change-name>`：生成或完善指定 OpenSpec change
- `/st:bridge`：把 `tasks.md` 转成桥接 todo
- `/st:plan`：只做计划
- `/st:apply`：进入交付
- `/st:archive-check`：检查是否满足归档条件
- `/st:archive`：执行归档

如果 AI 收到 `/st:*` 后没有进入工作流，而是回复“在吗”“有什么可以帮你”这类普通聊天内容，说明当前模型没有加载 `spectrace-rd` skill，或者没有识别 `/st:*` 协议。处理方式：

```text
请使用 spectrace-rd skill 处理下面的 /st:* 工作流命令。

/st:propose demand-addition-rate
...
如果仍然没有触发，需要确认当前 AI 客户端已经安装并启用了 `spectrace-rd` skill。
