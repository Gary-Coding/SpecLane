...
如果 AI 收到 `/sl:*` 后没有进入工作流，而是回复“在吗”“有什么可以帮你”这类普通聊天内容，说明当前模型没有加载 `speclane` skill，或者没有识别 `/sl:*` 协议。处理方式：

```text
请使用 speclane skill 处理下面的 /sl:* 工作流命令。
...
如果仍然没有触发，需要确认当前 AI 客户端已经安装并启用了 `speclane` skill。
