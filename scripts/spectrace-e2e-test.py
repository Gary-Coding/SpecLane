...
RUN_WORKFLOW = REPO_ROOT / "skills" / "spectrace-rd" / "scripts" / "run-workflow.py"
CLI = REPO_ROOT / "bin" / "spectrace.js"


def main() -> None:
    with tempfile.TemporaryDirectory(prefix="st-e2e-") as tmp:
        root = Path(tmp)
        home = root / "home"
        home.mkdir()
...
def test_templates_cli(root: Path) -> None:
    ...
    if "workspace.commands.st" not in doctor_before.output:
    ...
    if not (workspace / ".claude" / "commands" / "st" / "apply.md").exists():
        raise AssertionError("doctor --fix should create st command templates")
    run(["node", str(CLI), "commands", "install", "--workspace", str(workspace), "--target", "all"])
    for command_path in [
        workspace / ".cursor" / "commands" / "st" / "apply.md",
        workspace / ".trae" / "commands" / "st" / "apply.md",
        workspace / ".kimi" / "commands" / "st" / "apply.md",
        home_prompt(root) / "st-apply.md",
    ]:
...
def test_openspec_state_and_bridge(root: Path) -> None:
    ...
    if invalid.returncode == 0 or "请先执行 /st:propose" not in invalid.output:
        raise AssertionError("/st:bridge before /st:propose should be rejected")
    ...
    propose = run(
        [
            ...
            str(workspace),
            "--command-text",
            "/st:propose demo-change",
        ],
        env=env_without_openspec,
    )
    if "final_reply_must=代码未修改。下一步只能执行 /st:bridge。" not in propose:
        raise AssertionError("/st:propose did not print strict next-step constraint")
    ...
    if "bridge_generated=true" not in bridge:
        raise AssertionError("/st:bridge did not generate todo.md")
    ...
    if not route_payload.get("allowed") or route_payload.get("run_command") != "apply":
        raise AssertionError("route-check should return allowed JSON for bridged /st:apply")
    ...
    if stale_apply.returncode == 0 or "tasks.md 已变化" not in stale_apply.output:
        raise AssertionError("changed tasks.md should require /st:bridge before /st:apply")
    ...
    if "tasks_sha256=" not in bridge_again:
        raise AssertionError("second /st:bridge should record tasks hash")
    ...
    if "session_action=created" not in first_plan:
        raise AssertionError("first /st:plan should create a session")
    first_session = read_json(workspace / ".spectrace" / "current-session.json")
    ...
    second_session = read_json(workspace / ".spectrace" / "current-session.json")
    if "session_action=reused" not in second_plan or second_session["session_id"] != first_session_id:
        raise AssertionError("second /st:plan should reuse the existing planning session")
    if len(list((workspace / ".spectrace" / "sessions").iterdir())) != 1:
        raise AssertionError("repeated /st:plan created an extra session")
    ...
    if "apply_phase=implementing" not in apply_output:
        raise AssertionError("/st:apply should enter implementing after planned session")
    ...
    if "route_result_json_begin" not in apply_json_output.output and "当前状态不允许进入实现" not in apply_json_output.output:
        raise AssertionError("route-st --json should emit JSON summary or a state guard")
    blocked_plan = run(
        [
            sys.executable,
            str(RUN_WORKFLOW),
    ...
    final_session = read_json(workspace / ".spectrace" / "current-session.json")
    if blocked_plan.returncode == 0 or "不能重新执行 /st:plan" not in blocked_plan.output:
        raise AssertionError("/st:plan during active delivery should be rejected")
    if final_session["session_id"] != first_session_id or len(list((workspace / ".spectrace" / "sessions").iterdir())) != 1:
        raise AssertionError("rejected /st:plan should not create or switch sessions")
...
def test_todo_auto_session_and_verify_compaction(root: Path) -> None:
    ...
    if "apply_phase=implementing" not in apply_output:
        raise AssertionError("/st:apply did not enter implementing phase")

    run([sys.executable, str(RUN_WORKFLOW), "finish-implement", "--workspace", str(workspace)])
    session = read_json(workspace / ".spectrace" / "current-session.json")
...
def test_incomplete_plan_session_is_reused(root: Path) -> None:
    ...
    if first_apply.returncode == 0 or "未找到可识别的项目目录" not in first_apply.output:
        raise AssertionError("first /st:apply should fail before plan is generated")
    current = read_json(workspace / ".spectrace" / "current-session.json")
    first_session_id = current["session_id"]
    sessions_dir = workspace / ".spectrace" / "sessions"
    output_dir = workspace / "demands" / "10-broken" / "output"
    if len(list(sessions_dir.iterdir())) != 1:
        raise AssertionError("first failed /st:apply should create exactly one reusable data session")
    ...
    if second_apply.returncode == 0 or "session_action=reused_incomplete" not in second_apply.output:
        raise AssertionError("second /st:apply should reuse the incomplete planning session")
    current = read_json(workspace / ".spectrace" / "current-session.json")
    if current["session_id"] != first_session_id:
        raise AssertionError("incomplete planning session should remain current")
    if len(list(sessions_dir.iterdir())) != 1:
        raise AssertionError("repeated failed /st:apply should not create another data session")
    if output_dir.exists() and list(output_dir.iterdir()):
        raise AssertionError("repeated failed /st:apply should not create output report sessions")
