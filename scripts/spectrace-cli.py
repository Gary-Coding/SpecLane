...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="spectrace",
        description="SpecTrace workflow CLI.",
    )
    ...
    commands_install = commands_subparsers.add_parser("install", help="安装 /st:* 快捷命令模板。")
...
def run_setup(args: list[str]) -> None:
    script = REPO_ROOT / "scripts" / "spectrace-setup.py"
...
def doctor(workspace: Path, output_json: bool, fix: bool = False) -> int:
    ...
    add_check(checks, "workspace.commands.st", "ok" if workspace_commands_ready(workspace) else "warn", str(workspace / ".claude" / "commands" / "st"))
...
def workspace_commands_ready(workspace: Path) -> bool:
    commands_dir = workspace / ".claude" / "commands" / "st"
...
def command_target_dirs(workspace: Path, target: str) -> list[tuple[str, Path]]:
    ...
    mapping: dict[str, Path] = {
        "claude": workspace / ".claude" / "commands" / "st",
        "codex": Path(os.environ.get("CODEX_HOME", str(home / ".codex"))).expanduser() / "prompts",
        "cursor": workspace / ".cursor" / "commands" / "st",
        "trae": workspace / ".trae" / "commands" / "st",
        "kimi": workspace / ".kimi" / "commands" / "st",
    }
    if target == "all":
...
def install_commands(workspace: Path, target: str) -> None:
    ...
    for name, directory in command_target_dirs(workspace, target):
        ...
        for filename, content in ST_COMMANDS.items():
            output_name = f"st-{filename}" if name == "codex" else filename
...
def doctor_suggestions(checks: list[dict[str, str]]) -> list[str]:
    ...
    if by_name.get("codex_skill", {}).get("status") != "ok" or by_name.get("claude_skill", {}).get("status") != "ok":
        suggestions.append("执行 `spectrace sync --target both` 同步最新 skill。")
    if by_name.get("workspace.commands.st", {}).get("status") != "ok":
        suggestions.append("执行 `spectrace doctor --fix` 补齐工作区 `.claude/commands/st/*` 快捷命令。")
    if by_name.get("workspace_yml", {}).get("status") != "ok":
        suggestions.append("执行 `spectrace init` 初始化工作区，或用 `spectrace template copy <模板名>` 生成 workspace.yml。")
